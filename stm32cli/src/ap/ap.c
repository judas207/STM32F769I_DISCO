...
void apInit(void)
{
  logPrintf("stm32cli B20220105_R1\n\n");
...
void apMain(int argc, char *argv[])
{
  ...
  {
    logPrintf("stm32cli com1 115200 type[fw:bin] 0x08020000 file_name run[0:1]\n");
    logPrintf("stm32cli com1 115200 type[tag] file_name\n");
    apExit();
  }
  ...
  while(1)
  {
    ...
    pre_time = millis();
    err_code = bootCmdFlashErase(file_addr, file_size, 10000);
...
void ledControl(int argc, char *argv[])
{
  ...
  cmdOpen(&cmd, _DEF_UART2, uart_baud);
